...
-- NOTE: organization_id and campaign_id are TEXT in the usage table (Clerk IDs)

-- Drop existing function if it exists (with old UUID signature)
DROP FUNCTION IF EXISTS get_token_usage_summary(UUID, DATE, DATE, TEXT, UUID);
DROP FUNCTION IF EXISTS get_token_usage_summary(TEXT, DATE, DATE, TEXT, TEXT);

-- Create the function for daily token usage summary
-- Using TEXT for organization_id and campaign_id to match the usage table schema
...
DROP FUNCTION IF EXISTS get_token_usage_stats(TEXT, DATE, DATE, TEXT, TEXT);
