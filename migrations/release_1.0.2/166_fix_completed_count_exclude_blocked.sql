-- Migration: Fix completed count to exclude blocked_by_icp companies
-- Description: Update get_dashboard_stats function to match API logic:
--   - Completed should include: used_for_outreach = true OR processing_status = 'completed'
--   - But exclude companies where blocked_by_icp = true
-- Author: System
-- Date: 2025-11-20
    ...
    'companies', (
      SELECT jsonb_build_object(
        ...
        'completed', count(*) FILTER (
          WHERE (used_for_outreach = true OR processing_status = 'completed')
          AND COALESCE(blocked_by_icp, false) = false
        ),
...
COMMENT ON FUNCTION get_dashboard_stats(text) IS 'Provides unified dashboard statistics including tasks, contacts, and companies counts. Updated completed count to exclude blocked_by_icp companies and include processing_status = completed.';
