-- Migration: Fix cancelled companies count and add blocked_by_icp filter
-- Description: 
--   1. Fix cancelled count to only count companies where the latest company_verification task is cancelled
--   2. Add blocked_by_icp count to dashboard stats
-- Author: System
-- Date: 2025-01-XX
...
COMMENT ON FUNCTION get_dashboard_stats(text) IS 'Provides unified dashboard statistics including tasks, contacts, and companies counts. Fixed cancelled count to only count companies with latest cancelled task. Added blocked_by_icp count.';
