-- Migration: Simplify Campaign Companies Table
-- Description: Drops the existing complex campaign_companies table and recreates it as a simple many-to-many relationship
-- Author: System
-- Date: 2025-01-15

-- Drop the existing campaign_companies table with all its complexity
DROP TABLE IF EXISTS campaign_companies CASCADE;

-- Create a simple campaign_companies table with only essential fields
...
-- Create only the essential indexes
...
CREATE INDEX idx_campaign_companies_organization_id ON campaign_companies(organization_id);
